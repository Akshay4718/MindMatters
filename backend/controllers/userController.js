...
// Book appointment (modified to accept `onlineMode`)
const bookAppointment = async (req, res) => {
    try {
        const { userId, psyId, slotDate, slotTime, onlineMode } = req.body
        const psyData = await psychiatristModel.findById(psyId).select('-password')
        if (!psyData.available) {
            return res.json({ success: false, message: "Psychiatrist not available" })
        }
        let slots_booked = psyData.slots_booked
...
// Pseudo payment
const pseudoPayment = async (req, res) => {
    try {
        ...
        res.json({ success: true, message: "Pseudo payment successful" })
    } catch (error) {
        console.log(error)
        res.json({ success: false, message: error.message })
    }
